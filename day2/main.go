...
func main() {
	...
	for i := 0; i < len(input); i++ {
		spaceSplit := strings.Split(input[i], " ")
		nosStrings := strings.Split(spaceSplit[0], "-")

		nos := make([]int, len(nosStrings))

		for i, s := range nosStrings {
			nos[i], _ = strconv.Atoi(s)
		}

		searchCharAndPassword := []string{strings.Replace(spaceSplit[1], ":", "", -1), spaceSplit[2]}

		count := strings.Count(searchCharAndPassword[1], searchCharAndPassword[0])
		if count >= nos[0] && count <= nos[1] {
			valid += 1
		}

		r := []rune(searchCharAndPassword[1])
		first := nos[0]
		last := nos[1]
		if r[first-1] == []rune(searchCharAndPassword[0])[0] && r[last-1] != []rune(searchCharAndPassword[0])[0] || r[first-1] != []rune(searchCharAndPassword[0])[0] && r[last-1] == []rune(searchCharAndPassword[0])[0] {
			validPart2 += 1
	...
	fmt.Printf("Valid passwords for Part1: %v \n", valid)

	fmt.Printf("Valid passwords for Part2: %v \n", validPart2)
}
